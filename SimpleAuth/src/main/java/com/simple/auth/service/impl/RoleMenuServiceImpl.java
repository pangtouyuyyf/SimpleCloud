...
import com.simple.auth.service.RoleMenuService;
import com.simple.common.config.SysParams;
import com.simple.common.util.BinaryUtil;
...
public class RoleMenuServiceImpl implements RoleMenuService {
    ...
    public int saveRoleMenu(int currentUserId, JSONObject obj) {
        ...
        //查询现有角色菜单
        List<Map<String, Integer>> savedList = Optional.of(this.roleMenuDao.queryAll(param)).orElseGet(() -> new ArrayList<>());
        ...
        if (tempAddList != null && !tempAddList.isEmpty()) {
            ...
            String operateCode = BinaryUtil.reverseBinary(SysParams.Sys.OPERATE_CODE_DEFAULT, SysParams.Sys.OPERATE_CODE_LENGTH);
    ...
    public PageInfo queryRoleMenuOperate(int roleId, int page, int size) {
        ...
        String allCheckCode = BinaryUtil.toBinary((1 << SysParams.Sys.OPERATE_CODE_LENGTH) - 1, SysParams.Sys.OPERATE_CODE_LENGTH);
    ...
    public int saveRoleMenuOperate(int currentUserId, JSONObject obj) {
        ...
        //查询现有角色菜单操作
        List<Map<String, Object>> savedList = Optional.of(this.roleMenuDao.queryAllOperate(param)).orElseGet(() -> new ArrayList<>());
        ...
        for (int i = 0; i < operaArr.size(); i++) {
            ...
            roleMenu.put("operate_code", BinaryUtil.reverseBinary(temp.getInteger("value"), SysParams.Sys.OPERATE_CODE_LENGTH));
    ...
    private List<Map<String, Object>> analyzeOperateCode(String code) {
        ...
        String judge = null;
        for (int i = 0; i < SysParams.Sys.OPERATE_CODE_LENGTH; i++) {
            temp = new HashMap<>();
            temp.put("label", getOperateName(i));
            temp.put("value", 1 << i);
            judge = String.valueOf(code.charAt(i));
            if (SysParams.Common.YES.equals(judge)) {
                temp.put("checked", false);
            } else {
                temp.put("checked", true);
            }
